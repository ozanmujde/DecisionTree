...
class DecisionTreeClassifier:
    ...
    def predict(self, X: list[list[float]], root ):
        ...
        for i in range(len(X)):
            isCorrect, label = self.isDataLabeledCorrect(X[i], root)
    ...
    def splitTree(self, partArr: list[list[float]], question):
        trueList, falseList = [], []
        for j in range(len(partArr)):
            if partArr[j][question.att] <= question.number:
                trueList.append(partArr[j])
            else:
                falseList.append(partArr[j])
    ...
    def weightedGiniCalculator(self, trueList, falseList, gini):
        ...
        weightedGini = prob*trueGini + (1-prob)*falseGini
        return weightedGini
    ...
    def findQuestion(self, giniList: list[list[float]]):
        ...
        for index in range(4):
            for i in range(0, len(giniList)):
                question = Question(giniList[i][index], index)
                trueGini = 0
                falseGini = 0
    ...
    def isClassPure(self, countData: list[list[float]]) -> bool:
        ...
        if len(countData) == 0:
            return True
        label = countData[0][4]
        for i in range(1, len(countData)):
            if label != countData[i][4]:
                return False
        return True
    ...
    def constructTree(self, train_data: list[list[float]], depth: int):
        ...
        if self.isClassPure(train_data) == True:
